...
const _SceneObjectBase = class _SceneObjectBase {
  constructor(state) {
    ...
    this._refCount = 0;
    this._renderBeforeActivation = _SceneObjectBase.RENDER_BEFORE_ACTIVATION_DEFAULT;
    if (!state.key) {
      state.key = uuid.v4();
    }
  ...
  _activateBehavior(behavior) {
    if (behavior instanceof _SceneObjectBase) {
...
_SceneObjectBase.RENDER_BEFORE_ACTIVATION_DEFAULT = false;
let SceneObjectBase = _SceneObjectBase;
...
class SceneQueryController extends SceneObjectBase {
  constructor(state = {}, profiler) {
    ...
    this.addActivationHandler(() => {
      ...
      return () => {
        for (const entry of __privateGet$4(this, _running)) {
          this.changeRunningQueryCount(-1, entry);
        }
        __privateGet$4(this, _running).clear();
      };
    });
  }
  startProfile(name) {
...
const _MultiValueVariable = class _MultiValueVariable extends SceneObjectBase {
  ...
  getFieldAtPath(value, fieldPath, returnText = false) {
    ...
    if (Array.isArray(value)) {
      ...
      if (!isNaN(index) && index >= 0 && index < value.length) {
        ...
        return o2 ? o2.label : String(v);
      }
      return value.map((v) => {
        const o2 = this.state.options.find((o3) => o3.value === v);
        return o2 ? accesor(o2.properties) : v;
      });
    }
...
class ValueMacro {
  ...
  getValue(fieldPath) {
    ...
    if (fieldPath === "time") {
      ...
      return timeField ? timeField.values[rowIndex] : void 0;
    }
    if (!field) {
      return this._match;
    }
...
class ScopesVariable extends SceneObjectBase {
  ...
  validateAndUpdate() {
    if (!this.state.loading) {
      this._validateAndUpdateObs = void 0;
      return rxjs.of({});
    }
    this._validateAndUpdateObs = new rxjs.Subject();
  ...
  updateStateFromContext(state) {
    ...
    if (!loading && (scopesHaveChanged || newScopes.length === 0)) {
      ...
      this.publishEvent(new SceneVariableValueChangedEvent(this), true);
      (_a = this._validateAndUpdateObs) == null ? void 0 : _a.next({});
    } else {
...
function VizPanelSeriesLimit({ data, showAll, seriesLimit, onShowAllSeries }) {
  const styles = ui.useStyles2(getStyles$n);
...
const LazyLoader = React__default.default.forwardRef(
  ({ children, onLoad, onChange, className, mode = "mount", ...rest }, ref) => {
    ...
    const { hideEmpty } = ui.useStyles2(getStyles$m);
    ...
    return /* @__PURE__ */ React__default.default.createElement("div", { id, ref: innerRef, className: `${hideEmpty} ${className}`, ...rest }, !loaded && mode === "mount" ? (
      // eslint-disable-next-line @grafana/i18n/no-untranslated-strings
      "\xA0"
    ) : /* @__PURE__ */ React__default.default.createElement(LazyLoaderInViewContext.Provider, { value: isInView }, children));
  }
);
function getStyles$m() {
  return {
    hideEmpty: css.css({
      "&:empty": {
        display: "none"
...
function VizPanelRenderer({ model }) {
  ...
  let [ref, { width, height }] = reactUse.useMeasure();
  ...
  const plugin = (_a = model.getPlugin()) != null ? _a : getLoadingPlugin();
  const { dragClass, dragClassCancel } = getDragClasses(model);
  const dragHooks = getDragHooks(model);
  const dataObject = sceneGraph.getData(model);
  const rawData = dataObject.useState();
  const { series, annotations } = _UNSAFE_clearPreviousFieldValues ? (_b = rawData.data) != null ? _b : {} : {};
  ...
  if (dataWithFieldConfig.alertState) {
    titleItemsElement.push(
      /* @__PURE__ */ React__default.default.createElement(ui.Tooltip, { content: (_c = dataWithFieldConfig.alertState.state) != null ? _c : "unknown", key: `alert-states-icon-${model.state.key}` }, /* @__PURE__ */ React__default.default.createElement(
...
function useClearPreviousData(data) {
  ...
  const currVals = React.useRef();
  (_a = prevVals.current) != null ? _a : prevVals.current = /* @__PURE__ */ new Set();
  (_b = currVals.current) != null ? _b : currVals.current = /* @__PURE__ */ new Set();
  const currFrames = data;
  const prevFrames = reactUse.usePrevious(currFrames);
...
function getLoadingPlugin() {
  ...
  loadingPluginInstance = new data.PanelPlugin(LoadingPluginComp);
...
function seriesVisibilityConfigFactory(label, mode, fieldConfig, data) {
  if (mode === "setExactly") {
    const overrides2 = fieldConfig.overrides.filter((o) => !isHideSeriesOverride(o));
    if (label == null || Array.isArray(label) && label.length === getDisplayNames(data).length) {
      return { ...fieldConfig, overrides: overrides2 };
    }
    const override2 = createOverride$1(Array.isArray(label) ? label : [label]);
    return { ...fieldConfig, overrides: [...overrides2, override2] };
  }
  ...
  haveAlreadyTransformedData(data) {
    ...
    if (data.series === series && data.annotations === annotations) {
      if (this.state.data) {
        ...
        const metadataChanged = currentData.state !== nextData.state || ((_a = currentData.request) == null ? void 0 : _a.requestId) !== ((_b = nextData.request) == null ? void 0 : _b.requestId) || currentData.error !== nextData.error || !lodash.isEqual(currentData.errors, nextData.errors) || !lodash.isEqual(currentData.timeRange, nextData.timeRange);
        if (metadataChanged) {
...
class VizPanel extends SceneObjectBase {
  ...
  applyFieldConfig(rawData) {
    ...
    const newFrames = data.applyFieldOverrides({
      ...
      featureToggles: runtime.config.featureToggles
    });
...
function fuzzyFind(options, haystack, needle) {
  ...
  if (needle === "") {
    ...
    needle.length > maxNeedleLength || // skip fuzzy searching if too many terms
...
class AdHocFiltersVariableUrlSyncHandler {
  ...
  getUrlState() {
    ...
    if (originFilters == null ? void 0 : originFilters.length) {
      value.push(
        ...originFilters.filter((f) => isFilterComplete(f) && !f.hidden && f.origin && !isGroupByFilter(f) && f.restorable).map((f) => toArray(f).map(escapeOriginFilterUrlDelimiters).join("|").concat(`#${f.origin}#restorable`))
      );
      if (this._variable.isGroupByRestorable()) {
        ...
        if (activeGroupBys.length > 0) {
          value.push(
            ...activeGroupBys.map(
              (f) => toArray(f).map(escapeOriginFilterUrlDelimiters).join("|").concat(`#${f.origin}#restorable`)
            )
          );
        } else {
...
function updateOriginFilters(prevOriginFilters, filters) {
  ...
  const urlGroupByKeys = /* @__PURE__ */ new Set();
  ...
  for (let i = 0; i < filters.length; i++) {
    ...
    const isMatchedOrigin = foundOriginFilterIndex > -1 && filters[i].origin === prevOriginFilters[foundOriginFilterIndex].origin;
...
function toFilter(urlValue) {
  ...
  return {
    ...
    keyLabel,
    operator,
    value: (_a = values[0]) != null ? _a : "",
    values: isMultiValueOperator(operator) ? values.filter((_, index) => index % 2 === 0) : void 0,
    valueLabels: values.length > 0 ? values.filter((_, index) => index % 2 === 1) : void 0,
    condition: "",
...
const DropdownItem = React.forwardRef(
  function DropdownItem2({ children, active, addGroupBottomBorder, isMultiValueEdit, checked, nonInteractive, ...rest }, ref) {
...
const LoadingOptionsPlaceholder = () => {
  return /* @__PURE__ */ React__default.default.createElement(DropdownItem, { nonInteractive: true, onClick: (e) => e.stopPropagation() }, /* @__PURE__ */ React__default.default.createElement(i18n.Trans, { i18nKey: "grafana-scenes.variables.loading-options-placeholder.loading-options" }, "Loading options..."));
};
const NoOptionsPlaceholder = () => {
  return /* @__PURE__ */ React__default.default.createElement(DropdownItem, { nonInteractive: true, onClick: (e) => e.stopPropagation() }, /* @__PURE__ */ React__default.default.createElement(i18n.Trans, { i18nKey: "grafana-scenes.variables.no-options-placeholder.no-options-found" }, "No options found"));
};
const ExpressionHintPlaceholder = () => {
  return /* @__PURE__ */ React__default.default.createElement(DropdownItem, { nonInteractive: true, onClick: (e) => e.stopPropagation() }, /* @__PURE__ */ React__default.default.createElement(i18n.Trans, { i18nKey: "grafana-scenes.variables.expression-hint-placeholder.press-enter-to-apply" }, "Press Enter to apply"));
};
...
const INPUT_PLACEHOLDER_DEFAULT = "+ label = value";
const GROUP_BY_PLACEHOLDER_DEFAULT = "+ key";
const generatePlaceholder = (filter, filterInputType, isMultiValueEdit, isAlwaysWip, inputPlaceholder, isGroupBy) => {
  var _a;
  ...
  return filter[filterInputType] && !isAlwaysWip ? `${filter[filterInputType]}` : inputPlaceholder || defaultPlaceholder;
};
...
function parseFilterExpression(input, operatorValues) {
  ...
  const key = matchIndex > 0 ? trimmed.slice(0, matchIndex).trim() : void 0;
  if (matchIndex > 0 && !key) {
    return null;
  }
...
function useFreeFormExpression({
  controller,
  filter,
  inputValue,
  filterInputType,
  ...
  const commitExpressionUpdate = React.useCallback(() => {
    ...
    const keyLabel = filterInputType === "operator" ? (_a = filter.keyLabel) != null ? _a : filter.key : parsedExpression.key;
    if (!key) {
      return null;
    }
    const staged = { key, keyLabel, operator: parsedExpression.operator };
    if (!parsedExpression.value) {
      return staged;
    }
    if (isMultiValueOperator(parsedExpression.operator)) {
      const values = parsedExpression.value.split(",").map((v) => v.trim()).filter(Boolean);
      if (!values.length) {
        return staged;
      }
...
const AdHocCombobox = React.forwardRef(function AdHocCombobox2({
  ...
  const initialInputType = isAlwaysWip || isGroupBy ? "key" : "value";
  const [filterInputType, setInputType] = React.useState(initialInputType);
  const [preventFiltering, setPreventFiltering] = React.useState(initialInputType === "value");
  ...
  const getFirstSelectableIndex = React.useCallback((items) => {
  ...
  const handleFetchOptions = React.useCallback(
    async (inputType) => {
      ...
      try {
        if (inputType === "key") {
          if (isGroupBy && controller.getGroupByKeys) {
            options2 = await controller.getGroupByKeys(null);
          } else {
      ...
      (_d2 = controller.stopInteraction) == null ? void 0 : _d2.call(controller);
    },
    ...
    (event, multiValueEdit) => {
      if (event.key === "Backspace" && !inputValue) {
        ...
        if (filterInputType === "operator") {
          ...
          switchInputType("key", setInputType, void 0, refs.domReference.current);
          return;
        }
        if (isGroupBy && (filter == null ? void 0 : filter.origin)) {
          controller.updateToMatchAll(filter);
    ...
    (event, multiValueEdit) => {
      ...
      if (event.key === "Enter" && canCommitExpressionUpdate && filter) {
        ...
        if (parsed) {
          ...
          if (!parsed.value) {
            controller.updateFilter(filter, parsed);
            switchInputType("value", setInputType, void 0, refs.domReference.current);
            setInputValue("");
            setActiveIndex(null);
            return;
          }
          ...
          if (isAlwaysWip) {
            ...
            handleChangeViewMode == null ? void 0 : handleChangeViewMode(void 0, false);
            focusOnWipInputRef == null ? void 0 : focusOnWipInputRef();
          }
  ...
  return /* @__PURE__ */ React__default.default.createElement("div", { className: styles.comboboxWrapper }, filter && !isGroupBy ? /* @__PURE__ */ React__default.default.createElement("div", { className: styles.pillWrapper }, (filter == null ? void 0 : filter.key) ? /* @__PURE__ */ React__default.default.createElement("div", { className: css.cx(styles.basePill, styles.keyPill) }, keyLabel) : null, (filter == null ? void 0 : filter.key) && (filter == null ? void 0 : filter.operator) && filterInputType !== "operator" ? /* @__PURE__ */ React__default.default.createElement(
    "div",
    {
    ...
    "input",
    {
      ...getReferenceProps({
        ...
        id: (_b = controller.getControlId) == null ? void 0 : _b.call(controller),
        onChange,
        value: inputValue,
      ...
      className: css.cx(styles.inputStyle),
      onClick: (event) => {
        event.stopPropagation();
              ...
              ...getItemProps({
                ...
                onClick(event) {
                  ...
                  if (isGroupBy) {
                    event.stopPropagation();
                    if (isAlwaysWip) {
                      (_a3 = controller.addGroupByFilter) == null ? void 0 : _a3.call(controller, item);
                      handleResetWip();
                    } else {
                      controller.updateFilter(filter, {
                        key: item.value,
                  ...
                  if (isMultiValueEdit) {
                    ...
                    if (filterInputType === "value" && payload.value !== (filter == null ? void 0 : filter.value)) {
                      (_c2 = controller.startProfile) == null ? void 0 : _c2.call(controller, FILTER_CHANGED_INTERACTION);
                    }
                    controller.updateFilter(filter, payload);
                    populateInputValueOnInputTypeSwitch({
                      populateInputOnEdit,
...
const BasePill = React.forwardRef(function BasePill2({
  ...
  const pillText = /* @__PURE__ */ React__default.default.createElement("span", { className: css.cx(styles.pillText, strikethrough && styles.strikethrough) }, label);
  return /* @__PURE__ */ React__default.default.createElement(
    "div",
    {
      className: css.cx(
    ...
    label.length < LABEL_MAX_VISIBLE_LENGTH ? pillText : /* @__PURE__ */ React__default.default.createElement(ui.Tooltip, { content: /* @__PURE__ */ React__default.default.createElement("div", { className: styles.tooltipText }, label), placement: "top" }, pillText),
    removable && onRemove && /* @__PURE__ */ React__default.default.createElement(
      ui.IconButton,
      {
        onClick: (e) => {
          e.stopPropagation();
        ...
        name: "times",
        size: "md",
        className: css.cx(styles.pillIcon, disabled && styles.disabledPillIcon),
        tooltip: removeAriaLabel != null ? removeAriaLabel : i18n.t("grafana-scenes.components.base-pill.remove", "Remove {{label}}", { label })
      }
    ),
    additionalIcons
  );
...
const getStyles$f = getBasePillStyles;
...
function useEditablePill({
  ...
  const handlePillKeyDown = React.useCallback(
    (e) => {
      if (e.key === "Enter") {
        setPopulateInputOnEdit(true);
        handleChangeViewMode();
  ...
  if (viewMode) {
    ...
    return /* @__PURE__ */ React__default.default.createElement(
      BasePill,
      {
        ...
        removeAriaLabel: i18n.t(
          "grafana-scenes.components.adhoc-filter-pill.remove-filter-with-key",
          "Remove filter with key {{keyLabel}}",
          { keyLabel }
        ),
        removable: showRemove,
        additionalIcons: /* @__PURE__ */ React__default.default.createElement(React__default.default.Fragment, null, filter.origin && filter.readOnly && /* @__PURE__ */ React__default.default.createElement(
          ui.Tooltip,
          {
        ...
        ), filter.origin && cleanFilter && /* @__PURE__ */ React__default.default.createElement(ui.Tooltip, { content: getOriginFilterTooltips(filter.origin).info, placement: "bottom" }, /* @__PURE__ */ React__default.default.createElement(ui.Icon, { name: "info-circle", size: "md", className: styles.infoPillIcon })), filter.origin && filter.restorable && !filter.readOnly && /* @__PURE__ */ React__default.default.createElement(
          ui.IconButton,
          {
            onClick: (e) => {
              e.stopPropagation();
...
const GROUP_BY_WIP = { key: "", value: "", operator: "groupBy", condition: "" };
const AdHocFiltersAlwaysWipCombobox = React.forwardRef(function AdHocFiltersAlwaysWipCombobox2({ controller, onInputClick, isGroupBy }, parentRef) {
  ...
  if (viewMode) {
    return /* @__PURE__ */ React__default.default.createElement(
      BasePill,
      {
        ...
        onClick: handlePillClick,
        onKeyDown: handlePillKeyDown,
        ariaLabel: i18n.t("grafana-scenes.components.group-by-pill.group-by-key", "Group by {{keyLabel}}", { keyLabel }),
        onRemove: handleRemove,
        removable: !readOnly,
        removeAriaLabel: i18n.t(
        ...
        additionalIcons: /* @__PURE__ */ React__default.default.createElement(React__default.default.Fragment, null, isCleanDefault && /* @__PURE__ */ React__default.default.createElement(
          ui.Tooltip,
          {
            content: i18n.t(
...
const AdHocFiltersComboboxRenderer = React.memo(function AdHocFiltersComboboxRenderer2({ controller }) {
  ...
  const variableControlId = (_b = controller.getControlId) == null ? void 0 : _b.call(controller);
  React.useEffect(() => {
...
class AdHocFiltersVariable extends SceneObjectBase {
  ...
  _resolveOriginFlags(key, origin, values, operator, singleValue) {
    ...
    const isRestorable = !lodash.isEqual(values, original == null ? void 0 : original.value) || operator !== (original == null ? void 0 : original.operator);
    return {
  ...
  async _getGroupByKeys(currentKey, queries) {
    ...
    if (!ds || !ds.getGroupByKeys) {
      return override ? dataFromResponse(override.values).map(toSelectableValue$1) : [];
    }
    const applicableOriginFilters = (_d = (_c = this.state.originFilters) == null ? void 0 : _c.filter((f) => !f.nonApplicable && !isGroupByFilter(f) && !isMatchAllFilter(f))) != null ? _d : [];
    const otherFilters = this.state.filters.filter((f) => f.key !== currentKey && !f.nonApplicable && !isGroupByFilter(f) && !isMatchAllFilter(f)).concat((_e = this.state.baseFilters) != null ? _e : []).concat(applicableOriginFilters);
    const timeRange = sceneGraph.getTimeRange(this).state.value;
...
function DefaultGroupByCustomIndicatorContainer(props) {
  const { model } = props;
  const theme = ui.useTheme2();
  const styles = getStyles$9(theme);
...
class GroupByVariable extends MultiValueVariable {
  constructor(initialState) {
     ...
     *   instead of discovering all queries in the scene via getQueriesForVariables.
     */
    this._getKeys = async (ds, queries) => {
      var _a, _b, _c;
      ...
      const queriesForKeys = queries != null ? queries : getQueriesForVariables(this);
...
function WideMenu(props) {
  return /* @__PURE__ */ React__default.default.createElement(reactSelect.components.Menu, { ...props }, /* @__PURE__ */ React__default.default.createElement("div", { style: { minWidth: "220px" } }, props.children));
}
...
const MIXED_DATASOURCE_REF = { type: "mixed", uid: MIXED_DATASOURCE_UID };
function getNextRequestId$1(prefix = "SQR") {
  return prefix + counter$1++;
}
function isTemplatedDatasourceUid(uid) {
  if (!uid) {
    return false;
  }
  return /^(?:\$[A-Za-z0-9_]+|\$\{[^}]+\})/.test(uid);
}
function isTemplatedDatasourceRef(datasource) {
  return isTemplatedDatasourceUid(datasource == null ? void 0 : datasource.uid);
}
function getSimpleVariableNameFromDatasourceUid(uid) {
  if (!uid) {
    return void 0;
  }
...
function getDatasourceVariableSelectionCount(sceneObject, variableName) {
  const variable = sceneGraph.lookupVariable(variableName, sceneObject);
  if (!variable) {
    return void 0;
  }
...
function shouldUseMixedRuntimeDatasource(sceneObject, datasource, queries) {
  const refs = [datasource, ...queries.map((query) => query.datasource)];
  for (const ref of refs) {
    const uid = ref == null ? void 0 : ref.uid;
    if (!uid) {
      continue;
    }
    ...
    if (variableName) {
      ...
      if (selectionCount !== void 0 && selectionCount > 1) {
        return true;
      }
      continue;
    }
...
class SceneQueryRunner extends SceneObjectBase {
  constructor(initialState) {
    ...
    this.onDataReceived = (data$1) => {
      ...
      if (last != null && dataWithLayersApplied.state !== schema.LoadingState.Streaming) {
        const sameSeries = data.compareArrayValues((_a = last.series) != null ? _a : [], (_b = dataWithLayersApplied.series) != null ? _b : [], (a, b) => a === b);
        const sameAnnotations = data.compareArrayValues(
          (_c = last.annotations) != null ? _c : [],
          (_d = dataWithLayersApplied.annotations) != null ? _d : [],
          (a, b) => a === b
        );
        const sameState = last.state === dataWithLayersApplied.state;
        const sameErrors = data.compareArrayValues(
          (_e = last.errors) != null ? _e : [],
          (_f = dataWithLayersApplied.errors) != null ? _f : [],
          (a, b) => lodash.isEqual(a, b)
        );
        if (sameSeries) {
        ...
        if (sameSeries && sameAnnotations && sameState && sameErrors) {
          if (((_g = last.request) == null ? void 0 : _g.requestId) === ((_h = dataWithLayersApplied.request) == null ? void 0 : _h.requestId)) {
            return;
          }
        }
      }
  ...
  _onDeactivate() {
    ...
    if (this.state.data && ![schema.LoadingState.Done, schema.LoadingState.Error].includes(this.state.data.state)) {
      this.setState({ data: void 0 });
    }
  ...
  cancelQuery() {
    ...
    this.setState({
      data: { ...(_b = this.state.data) != null ? _b : emptyPanelData, state: schema.LoadingState.Done }
    });
  }
  async runWithTimeRange(timeRange) {
    ...
    try {
      ...
      const runtimeDatasource = shouldUseMixedRuntimeDatasource(this, datasource, queries) ? MIXED_DATASOURCE_REF : datasource;
  ...
  prepareRequests(timeRange, ds) {
    ...
    let request = {
      app: "scenes",
      requestId: getNextRequestId$1(requestIdPrefix),
    ...
    request.targets = request.targets.map((query) => {
      ...
      if (!query.datasource || query.datasource.uid !== ds.uid && !((_a2 = ds.meta) == null ? void 0 : _a2.mixed) && runtime.isExpressionReference && !runtime.isExpressionReference(query.datasource)) {
        if (!query.datasource && ((_b = ds.meta) == null ? void 0 : _b.mixed) && isTemplatedDatasourceRef(this.state.datasource)) {
    ...
    for (const provider of (_a = this.getClosestExtraQueryProviders()) != null ? _a : []) {
      for (const { req, processor } of provider.getExtraQueries(request)) {
        const requestId = getNextRequestId$1(requestIdPrefix);
...
function getQueriesForVariables(sourceObject) {
  ...
  const applicableRunners = filterOutInactiveRunnerDuplicates(runners).filter((r) => {
    var _a2;
    if ((_a2 = r.state.datasource) == null ? void 0 : _a2.uid) {
    ...
    return r.state.queries.some((q) => {
      var _a3;
      if (!((_a3 = q.datasource) == null ? void 0 : _a3.uid)) {
        return false;
      }
  ...
  return (source) => source.pipe(
    operators.mergeMap((data$1) => {
      ...
      return rxjs.of(data$1).pipe(
        data.standardTransformersRegistry.get("merge").transformation.operator({}, ctx),
        operators.map((d) => d[0])
      );
    })
  );
...
class AnnotationsDataLayer extends SceneDataLayerBase {
  ...
  async runWithTimeRange(timeRange) {
    ...
    this._drilldownDependenciesManager.findAndSubscribeToDrilldowns((_a = query.datasource) == null ? void 0 : _a.uid, this);
    if (this.querySub) {
      this.querySub.unsubscribe();
    }
    if (this._variableDependency.hasDependencyInLoadingState()) {
...
function toMetricFindValues(valueProp, textProp) {
  return (source) => source.pipe(
    rxjs.map((panelData) => {
      ...
      for (const frame of frames) {
        for (let index = 0; index < frame.length; index++) {
          const fieldValue = (fieldIndex) => fieldIndex !== -1 ? frame.fields[fieldIndex].values[index] : void 0;
...
function findFieldsIndices(frames, valueProp, textProp) {
  ...
  for (const frame of data.getProcessedDataFrames(frames)) {
    for (let index = 0; index < frame.fields.length; index++) {
      ...
      if (field.type === data.FieldType.string || field.type === data.FieldType.number) {
  ...
  indices.properties = lodash.uniqWith(indices.properties, lodash.isEqual);
  return indices;
}
...
class UrlSyncManager {
  constructor(_options = {}, locationService = runtime.locationService) {
    this._lastSyncedObjectKeys = /* @__PURE__ */ new WeakMap();
    this._options = _options;
  ...
  handleSceneObjectStateChanged(changedObject) {
    ...
    const previousKeys = (_a = this._lastSyncedObjectKeys.get(changedObject)) != null ? _a : [];
    ...
    if (Object.keys(mappedUpdated).length > 0) {
      const shouldCreateHistoryEntry = (_c = (_b = changedObject.urlSync).shouldCreateHistoryStep) == null ? void 0 : _c.call(_b, newUrlState);
...
function SceneGridLayoutRenderer({ model }) {
  ...
  const renderGrid = (width2, height2) => {
    if (!width2 || model.state.UNSAFE_fitPanels && !height2) {
      return null;
    }
    ...
    return (
      ...
      /* @__PURE__ */ React__default.default.createElement(
        ...
        {
          ref,
          style: { width: `${width2}px`, height: layout.length > 0 ? "100%" : "0" },
...
function SceneGridRowRenderer({ model }) {
  ...
  return /* @__PURE__ */ React__default.default.createElement("div", { className: css.cx(styles.row, isCollapsed && styles.rowCollapsed) }, /* @__PURE__ */ React__default.default.createElement("div", { className: styles.rowTitleAndActionsGroup }, /* @__PURE__ */ React__default.default.createElement(
    "button",
    {
      ...
      "aria-label": isCollapsed ? i18n.t("grafana-scenes.components.scene-grid-row.expand-row", "Expand row with title {{title}}", { title }) : i18n.t("grafana-scenes.components.scene-grid-row.collapse-row", "Collapse row with title {{title}}", {
        title
      }),
...
class SceneTimePicker extends SceneObjectBase {
  constructor() {
    ...
    this.onMoveBackward = () => {
      ...
      const newTimeRange = getShiftedTimeRange(0 /* Backward */, range);
    ...
    this.onMoveForward = () => {
      ...
      const newTimeRange = getShiftedTimeRange(1 /* Forward */, range);
      if (isValidTimeRange(newTimeRange)) {
        timeRange.onTimeRangeChange(newTimeRange);
      }
    };
...
function SceneRefreshPickerRenderer({ model }) {
  ...
  let loadingText;
  if (withText) {
    loadingText = i18n.t("grafana-scenes.components.scene-refresh-picker.text-cancel", "Cancel");
  }
  if (isRunning) {
...
function SceneCSSGridLayoutRenderer({ model }) {
  ...
  return /* @__PURE__ */ React__default.default.createElement("div", { className: style }, children.map((item) => {
    const Component = item.Component;
    if (isLazy) {
      return /* @__PURE__ */ React__default.default.createElement(LazyLoader, { key: item.state.key, className: style, mode: "query" }, /* @__PURE__ */ React__default.default.createElement(Component, { key: item.state.key, model: item, parentState: model.state }));
    }
    return /* @__PURE__ */ React__default.default.createElement(Component, { key: item.state.key, model: item, parentState: model.state });
  }));
}
...
function __variableDynamicImportRuntime0__(path) {
  switch (path) {
    ...
    case '../locales/en-US/grafana-scenes.json': return Promise.resolve().then(function () { return require('./grafana-scenes-kW_VuTmG.js'); });
...
exports.GRID_CELL_HEIGHT = GRID_CELL_HEIGHT;
exports.GRID_COLUMN_COUNT = GRID_COLUMN_COUNT;
exports.GROUP_BY_OPERATOR = GROUP_BY_OPERATOR;
...
exports.isGroupByFilter = isGroupByFilter;
